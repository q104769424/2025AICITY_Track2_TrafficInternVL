...
parser.add_argument('--save-folder', type=str, default='processed_anno/qa_format', help='dirname for saving json file')
...
output_path = os.path.join(args.save_folder, f'wts_bdd_{args.split}.json')
with open(output_path, 'w+') as f:
    f.write(json.dumps(reserved_train_samples, indent=4))

processing_stats = {
    'total_samples': len(reserved_train_samples),
    'WTS': {
        'overhead': 0,
        'vehicle': 0,
...
for sample in reserved_train_samples:
    image_path = sample['image']
    dataset = 'WTS' if 'WTS' in image_path else 'BDD'
    view = sample['view'] if 'view' in sample else ''

    if dataset == 'WTS':
        if 'normal_trimmed' in image_path:
            if view == 'overhead':
        ...
        processing_stats['BDD'][segment] += 1
...
for phrase in phrase_number_map.values():
    print(f"    - {phrase.capitalize()}: {processing_stats['BDD'].get(phrase, 0)} samples")
