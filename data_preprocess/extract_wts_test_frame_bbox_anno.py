...
parser.add_argument('--root', type=str, default='data/test_part', help='data root path')
parser.add_argument('--save-folder', type=str, default='processed_anno', help='dirname for saving json file')

args = parser.parse_args()

video_path = os.path.join(args.root, 'SubTask1-Caption/WTS_DATASET_PUBLIC_TEST/videos/test/public')
annotation_path = os.path.join(args.root, 'SubTask1-Caption/WTS_DATASET_PUBLIC_TEST/annotations/caption/test/public_challenge')
bbox_path = os.path.join(args.root, 'SubTask1-Caption/WTS_DATASET_PUBLIC_TEST_BBOX/annotations/bbox_annotated/')
...
for item in tqdm(os.listdir(video_path)):
    ...
    for view in ['overhead', 'vehicle']:
        ...
        for camera in os.listdir(current_view):
            ...
            pedestrian_bbox_anno_path = os.path.join(bbox_path, 'pedestrian', 'test/public', item, f'{view}_view', f'{camera_base}_bbox.json')
            ...
            vehicle_bbox_anno_path = os.path.join(bbox_path, 'vehicle', 'test/public', item, f'{view}_view', f'{camera_base}_bbox.json')
...
with open(os.path.join(args.save_folder, 'wts_test_all_video_with_bbox_anno_first_frame.json'), 'w') as f:
